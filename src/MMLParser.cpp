...
MMLParser *MMLParser::pFirstMMLParser = nullptr;
MMLParser *MMLParser::pPrevMMLParser = nullptr;
...
void MMLParser::startup(bool isContinue) {
	p = pStart;
...
int MMLParser::translate(MMLPTR mml, bool isContinue) {
	isTranslate = true;
	pStart = mml;
	translateCount = 0;
	startup(isContinue);

	unsigned long tick = TIMEBASE;
	do {
		update(tick);
		tick += stepToTime(steps);
	} while(p);
	return totalSteps;
}
...
void MMLParser::noteOn(int num, int velocity) {
	if (pfnCallback) {
		...
		callback2(num, velocity, steps, stepsGate);
		translateCount++;
	}
	prevNum = num;
}
...
void MMLParser::noteOff() {
	...
	} else if (steps && stepsGate == 0) {
		callback2(0, 0, steps, stepsGate);
		translateCount++;
	}
}
...
bool MMLParser::update(unsigned long tick) {
	...
	if (steps) {
		int e = timeToStep((int)(tick - startTick)) - totalSteps;
		if (stepsGate) {
			if (e >= stepsGate) {
				noteOff();
				stepsGate = 0;
